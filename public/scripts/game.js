window.onload = function() {
  var SPRITES = '/img/crafty-sprite.png';
	...
	Crafty.scene("main", function() {
		...
		var player = Crafty.e("2D, Canvas, player, SpriteAnimation, Fourway, Collision")
		...
		player.bind('Moved', function() { updatePosition([this.x, this.y]) });
	});
	...
	socket.on('user connected', function(data)
	{
		...
		{
			xx[data.id] = Crafty.e("2D, Canvas, player, SpriteAnimation, Fourway, Collision")
			.attr({x: data.pos[0], y: data.pos[1], z: 1});
		}
		else
			xx[data.id].attr({x: data.pos[0], y: data.pos[1], z: 1});
	});
